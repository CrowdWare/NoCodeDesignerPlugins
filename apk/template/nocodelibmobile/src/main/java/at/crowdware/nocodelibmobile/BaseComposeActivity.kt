...
abstract class BaseComposeActivity : ComponentActivity() {
    ...
    private fun installCacheFromAssets() {
        val directory = File(this.filesDir, "ContentCache/crowdware_github_io/NoCode")
        var pages: File
        var parts: File
        var images: File
        var sounds: File
        ...
        if (directory.exists()) {
            ...
            textures.mkdir()
            translations = File(directory, "translations")
            translations.mkdir()
        }
        ...
        try {
            ...
            copyDir("pages", pages)
            copyDir("parts", parts)
            copyDir("models", models)
            copyDir("textures", textures)
            copyDir("translations", translations)
            copyFile("app.sml", "app.sml", directory)
        } catch(e: Exception) {
